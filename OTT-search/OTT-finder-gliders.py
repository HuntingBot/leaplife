...
with open(output_fname, "w") as outf:
  outf.write("x = 0, y = 0, rule = B2n3/S23-q\n")
...
turner, split2, split3, split4, splitmore = 0, 0, 0, 0, 0
for item in constellations:
  ...
  for lane in range(mindiag - 6 + HEIGHT + 2, maxdiag + 6 + HEIGHT + 2):
    count += 1
    ...
    if count % 1000 == 0:
      g.show("Matches so far: " + str([match, turner, split2, split3, split4, splitmore]))
      g.fit()
      g.update()
    ...
    if len(remainder) == 0:
      ...
      if pop !=0:
        ...
        if pop % 5 == 0:
          ...
          #   (some very rare combinations of *WSSes or lepas may get thrown out with this test,
          #    but probably not!)
        else:
          justgliders = 0
        if justgliders == 1:
